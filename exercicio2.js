  ...
  */
  ...
  async function converterDolarParaReal(precoEmDolar) {
    try {
      const cotacaoDolar = await buscarPrecoDolar();
      const juros = await buscarJurosImportacao();
      const valorReal = cotacaoDolar.comercial * precoEmDolar;
      const valorFinal = valorReal + (valorReal * juros.juros1) + (valorReal * juros.juros2);
  
      return console.log(`O preço final do seu produto é R$ ${valorFinal.toFixed(2).replace(".",",")}`);
    } 
    catch (error) {
      console.log(error)
    }
  ...
  converterDolarParaReal(precoEmDolar) 