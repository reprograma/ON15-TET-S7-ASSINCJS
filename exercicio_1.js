...
async function resolverPromisses(produto) {
  try {
      // produto == "lace"
      const nomeProduto = await buscarPreco(produto);
      const parcelaProduto = await calcularParcela(nomeProduto.preco);
      // `Sua Lace custa R$3900,00 e você pagará em 10x de R$390,00`
      console.log (`
      Sua ${nomeProduto.nome} custa R$ ${(nomeProduto.preco).toFixed(2).replace(".", ",")} e você pagará em 10x de R$ ${(parcelaProduto).toFixed(2).replace(".", ",")}
      `);
