...
const buscarPreco = (produto) => {
  ...
  function calcularParcela (preco) {
   ...
   async function calcularLace(produto) {
    try {
      const item = await buscarPreco(produto)
      const calParc = await calcularParcela(item.preco)
      return console.log(`
    Sua Lace custa R$ ${item.preco.toFixed(2)} e você pagará em 10x de R$ ${calParc.toFixed(2)}
    `)
    }catch(error){
...
// Precisamos calcular o valor da parcela de uma Lace de cabelo humano que a Byanka Nicoli decidiu comprar, 
// para isso precisamos 
// descobrir o preço do produto e em seguida o número de parcelas desejadas

// retorno no console: `Sua Lace custa R$3900,00
// e você pagará em 10x de R$390,00`
// */
